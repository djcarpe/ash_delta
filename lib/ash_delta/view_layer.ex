  ...
  @delta_view %Spark.Dsl.Section{
    ...
    schema: [
      ...
      sources: [
        ...
        required: true,
        doc: """
        Source resources referenced by `query`. Accepts either a plain list of
        modules (`sources [MyApp.Events]`) or a keyword list of aliases
        (`sources source: MyApp.Events, other: MyApp.Other`). In the plain-list
        form the first module gets the alias `source`; subsequent modules use
        their lowercased short name. Each alias becomes a table name in the SQL.
        All sources must use `AshDelta.DataLayer` or `AshDelta.ViewLayer`.
        """
      ],
      query: [
        type: :string,
