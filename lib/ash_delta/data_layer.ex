defmodule AshDelta.DataLayer do
  ...
  alias AshDelta.{BufferedWriter, Info, Rewrite, Writer}
  ...
  def can?(_, {:query_aggregate, kind}) when kind in [:count, :sum, :avg, :min, :max], do: true
  ...
  defp write_records(resource, records) do
    result =
      case Info.write_buffer_ms(resource) do
        ...
        {:ok, ms}  -> BufferedWriter.append(resource, records, ms)
      end
      ...
      {:ok, threshold} ->
        ...
        Task.start(fn ->
          Enum.each(partitions, fn partition ->
            ...
            lock_key = {resource, string_partition}

            if length(live) >= threshold and :ets.insert_new(:ash_delta_compacting, {lock_key, true}) do
              try do
                AshDelta.Maintenance.optimize(resource, partition: partition, min_files: threshold)
              after
                :ets.delete(:ash_delta_compacting, lock_key)
              end
            end
          end)
        end)
