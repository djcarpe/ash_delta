defmodule AshDelta.Rewrite do
  ...
  alias AshDelta.{ConnectionPool, DataLayer.Query, Log, Pruner, Reader, Sql, Writer}
  ...
  defp plan_file(resource, file, where_sql, params, transform) do
    ...
    {:ok, conn} = ConnectionPool.checkout(resource)
  ...
  defp normalize_filter(resource, filter) when is_list(filter) do
    Ash.Filter.parse!(resource, filter)
  end

  defp normalize_filter(resource, filter) when is_map(filter) and not is_struct(filter) do
    Ash.Filter.parse!(resource, Enum.to_list(filter))
  end

  defp normalize_filter(resource, expr) do
    Ash.Filter.parse!(resource, expr)
  end
end
